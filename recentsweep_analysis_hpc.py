...
cF = sys.argv[5] ## conditional frequency
...
for s in [0.02,0.05,0.1,0.2]: ## test varying s
   #check if simultion file already existis, if not simulate demography
    ...
    if os.path.isfile("{0}simplegross_{1}.trees".format(path,params))==False:
        ...
        cmd = "slim -d s=" + str(s) + " -d rep="+str(rep)+" -d admix="+ str(admixture)+" -d sampleSize=" + str(sampleSize)+ " -d selPop=" + str(selPop)+ " -d selTime=" + str(selTime) +" -d selEnd=" + str(selEnd)  + " -d cF=" + str(cF)+ ' -d "' + str(results) + '" /storage/home/olj5016/work/arg_selection/old_slim.slim' #make sure path to slim file is correct
        print(cmd)
        os.system(cmd)
    ...
    # export table
    windows.to_csv( "recentLR_{0}.txt".format(params), sep="\t", index=False)
